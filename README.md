...
- 🪟 **界面与 DSH Web GUI 一致**：启动页/状态页使用同一套设计 token，亮/暗主题与客户端当前主题对齐（读取 DSH 设置的 `ui-theme.preference`，未设置时跟随系统）；隐藏系统标题栏，自绘窗口按钮浮于界面右上角，颜色与悬停高亮复用 DSH 设计 token，与界面其他组件一致；内容区顶部注入透明拖拽条用于移动窗口；启动页保持覆盖到 DSH 界面渲染完成才切换，启动过程无白屏闪烁。
...
├── splash/              启动/状态/错误页（与 DSH Web GUI 同款视觉，主题跟随客户端）
├── assets/              图标资源（DeepSeek 官方 logo，多尺寸 PNG/ICO）
├── test/                单元测试与冒烟脚本
├── package.json
